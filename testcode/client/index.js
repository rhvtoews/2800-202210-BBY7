...
//   const fullname = nameInput.ariaValueMax;
//   nameInput.value = "";
...
//   }).then(response => response.json()).then(data => placeRow());
// }
...
// }
  ...
  data.forEach(function ({ID, fullname, email, password, city, admin }) {
    adminTable += `<td>${ID}</td>`;
    adminTable += `<td>${fullname}</td>`;
    adminTable += `<td>${email}</td>`;
    adminTable += `<td>${password}</td>`;
    adminTable += `<td>${city}</td>`;
  ...
  if (hadTableData) {
    ...
    const newRow = table.placeRow();
    newRow.innerHTML = adminTable;
  }
